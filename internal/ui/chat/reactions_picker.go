...
func newReactionsPicker(cfg *config.Config, chatView *Model) *reactionsPicker {
	rp := &reactionsPicker{
...
func (rp *reactionsPicker) update(message discord.Message) {
	...
	if message.GuildID.IsValid() {
		emojis, err := rp.chatView.state.Cabinet.Emojis(message.GuildID)
		if err != nil {
...
func (rp *reactionsPicker) close() {
	rp.chatView.RemoveLayer(reactionsPickerLayerName)
	rp.chatView.app.SetFocus(rp.chatView.messagesList)
}

func (rp *reactionsPicker) HandleEvent(event tview.Event) tview.Command {
	...
	case *picker.SelectedEvent:
		...
		option := rp.options[index]
		state := rp.chatView.state
	...
	return rp.Model.HandleEvent(event)
}
